...
```
    *   Save and exit the editor.
        ...
        *   If you see an error like **"bad minute"**, it means there is a typo or an extra line break. The most common mistake is having a line break between the schedule (`0 5 * * *`) and the command (`curl...`). **Each job must be on a single, continuous line.** Re-open with `crontab -e` and carefully check your work.
...
*   `0`: **Minute** (0-59). This runs at the 0th minute (i.e., on the hour).
