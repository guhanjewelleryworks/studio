...
import { MapPin, List, Search, Star, Loader2, SlidersHorizontal, Building, Palette, LinkIcon } from 'lucide-react';
...
const MapPlaceholder = ({ locations }: { locations: Location[] }) => (
  <div className="w-full h-[300px] md:h-full bg-muted/30 rounded-lg flex flex-col items-center justify-center text-muted-foreground border border-border shadow-inner">
    <MapPin className="h-12 w-12 text-primary/70 mb-3" />
...
export default function DiscoverPage() {
  ...
  return (
    <div className="container py-4 px-4 md:px-6 min-h-[calc(100vh-6rem)] bg-background text-foreground">
      <div className="flex flex-col md:flex-row items-center justify-between gap-3 mb-5">
        <h1 className="text-3xl font-extrabold text-primary tracking-tight">Find a Goldsmith</h1>
        <div className="flex gap-2">
          <Button variant={viewMode === 'list' ? 'default' : 'outline'} onClick={() => setViewMode('list')} aria-label="List View" className="rounded-lg px-4 py-2 text-sm shadow-sm">
            <List className="mr-1.5 h-4 w-4" /> List
          </Button>
          <Button variant={viewMode === 'map' ? 'default' : 'outline'} onClick={() => setViewMode('map')} aria-label="Map View" className="rounded-lg px-4 py-2 text-sm shadow-sm">
            <MapPin className="mr-1.5 h-4 w-4" /> Map
          </Button>
        </div>
      </div>

      <form onSubmit={handleSearch} className="mb-5 flex flex-col sm:flex-row gap-2 items-center">
        <Input
          type="search"
          placeholder="Search by name, specialty, or location..."
          className="flex-grow rounded-lg text-sm px-4 py-2 shadow-sm border-border focus:ring-primary focus:border-primary"
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
        />
        <Button type="button" variant="outline" size="icon" className="sm:ml-1 hidden sm:inline-flex p-2 rounded-lg shadow-sm border-border hover:bg-muted/50">
            <SlidersHorizontal className="h-4 w-4 text-muted-foreground" />
            <span className="sr-only">Filters</span>
        </Button>
        <Button type="submit" disabled={isLoading} size="default" className="rounded-lg px-5 py-2 text-sm w-full sm:w-auto shadow-sm bg-primary hover:bg-primary/90 text-primary-foreground">
      ...
      <div className={`grid gap-3.5 ${viewMode === 'map' ? 'grid-cols-1 md:grid-cols-[2fr_3fr]' : ''}`}>
        <div className={`${viewMode === 'map' ? 'hidden md:block' : ''} ${viewMode === 'list' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3' : 'overflow-y-auto max-h-[calc(100vh-18rem)] pr-1.5 space-y-2.5'}`}>
          {isLoading ? (
             Array.from({ length: viewMode === 'list' ? 8 : 3 }).map((_, index) => (
              <Card key={index} className="animate-pulse bg-muted/50 h-[320px] rounded-xl shadow-md border-border"></Card>
             ))
          ) : goldsmiths.length > 0 ? (
             goldsmiths.map((goldsmith) => (
              <Card key={goldsmith.id} className="shadow-lg hover:shadow-xl transition-all duration-300 bg-card border-primary/20 flex flex-col rounded-xl overflow-hidden group">
                <CardHeader className="p-0 relative">
                  <Image
                    ...
                    width={400}
                    height={200}
                    className="rounded-t-xl object-cover w-full aspect-[2/1] group-hover:scale-105 transition-transform duration-300"
                ...
                <CardContent className="p-3 flex-grow flex flex-col justify-between">
                  <div>
                    <CardTitle className="text-lg text-primary-foreground mb-0.5 group-hover:text-primary transition-colors">{goldsmith.name}</CardTitle>
                    ...
                    <p className="text-xs text-foreground/80 mb-1 font-medium flex items-center">
                      <Palette className="inline-block h-3 w-3 mr-1 text-accent"/>Specialty: {goldsmith.specialty}
                    </p>
                    <p className="text-sm text-muted-foreground leading-snug line-clamp-2 mb-2">{goldsmith.shortBio}</p>
                  </div>
                  <Link
                    href={`/goldsmith/${goldsmith.id}`}
                    className={cn(
                       buttonVariants({ variant: "outline", size: "sm" }),
                       'text-accent-foreground border-accent hover:bg-accent/20 mt-1.5 w-full rounded-lg text-xs py-1.5 shadow-sm'
                    )}
                  >
             ...
             !isLoading && !error && <p className="col-span-full text-center text-muted-foreground py-6 text-base">No goldsmiths found based on your criteria.</p>
          )}
        </div>
