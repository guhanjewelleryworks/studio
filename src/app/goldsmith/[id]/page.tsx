...
export default function GoldsmithProfilePage({ params }: { params: PageParams }) {
  // Ensure params are resolved using React.use()
  ...
  if (isLoading) {
     return (
      <div className="container py-6 px-4 md:px-6 min-h-[calc(100vh-8rem)] bg-background text-foreground"> {/* Reduced py */}
        <div className="grid md:grid-cols-3 gap-6"> {/* Reduced gap */}
          <div className="md:col-span-1 space-y-5"> {/* Reduced space-y */}
            <Skeleton className="h-32 w-32 rounded-full mx-auto bg-muted/70" /> {/* Reduced h */}
            <Skeleton className="h-7 w-4/5 mx-auto bg-muted/70" /> {/* Reduced h */}
            <Skeleton className="h-5 w-3/5 mx-auto bg-muted/70" /> {/* Reduced h */}
            <Skeleton className="h-4 w-2/5 mx-auto bg-muted/70" /> {/* Reduced h */}
            <Skeleton className="h-10 w-full rounded-full bg-muted/70" /> {/* Reduced h */}
            <Skeleton className="h-10 w-full rounded-full bg-muted/60" /> {/* Reduced h */}
          </div>
          <div className="md:col-span-2 space-y-6"> {/* Reduced space-y */}
             <Skeleton className="h-9 w-2/5 bg-muted/70" /> {/* Reduced h */}
             <Skeleton className="h-20 w-full bg-muted/70" /> {/* Reduced h */}
             <Skeleton className="h-9 w-1/3 bg-muted/70" /> {/* Reduced h */}
             <div className="grid grid-cols-2 sm:grid-cols-3 gap-3"> {/* Reduced gap */}
                 <Skeleton className="h-40 w-full aspect-square rounded-lg bg-muted/70" /> {/* Reduced h */}
                 <Skeleton className="h-40 w-full aspect-square rounded-lg bg-muted/70" /> {/* Reduced h */}
                 <Skeleton className="h-40 w-full aspect-square rounded-lg bg-muted/70" /> {/* Reduced h */}
             </div>
             <Skeleton className="h-9 w-1/3 bg-muted/70" /> {/* Reduced h */}
              <Skeleton className="h-40 w-full rounded-lg bg-muted/70" /> {/* Reduced h */}
          </div>
        </div>
      </div>
     );
   ...
   if (error) {
     return <div className="container py-6 md:py-10 px-4 md:px-6 text-center text-destructive text-lg">{error}</div>; {/* Reduced py */}
   }

  if (!profile) {
    return <div className="container py-6 md:py-10 px-4 md:px-6 text-center text-muted-foreground text-lg">Profile not found.</div>;  {/* Reduced py */}
  }
      ...
      <div className="grid md:grid-cols-3 gap-6 lg:gap-8"> {/* Reduced gap */}
        ...
        <div className="md:col-span-1 space-y-5"> {/* Reduced space-y */}
          <Card className="shadow-xl border-primary/15 rounded-xl overflow-hidden bg-card">
            <CardHeader className="items-center text-center bg-gradient-to-b from-card to-secondary/10 p-5"> {/* Reduced p */}
              <Avatar className="w-28 h-28 mb-3 border-4 border-primary/50 shadow-lg"> {/* Reduced size and mb */}
                <AvatarImage src={profile.profileImageUrl} alt={profile.name} data-ai-hint="artisan portrait" />
                <AvatarFallback className="text-3xl bg-primary/20 text-primary-foreground font-poppins">{profile.name.charAt(0)}</AvatarFallback> {/* Reduced text size */}
              </Avatar>
              <CardTitle className="text-xl text-foreground">{profile.name}</CardTitle> {/* Reduced text size */}
              {profile.tagline && <p className="text-xs text-primary font-medium font-poppins mt-0.5">{profile.tagline}</p>} {/* Reduced text size */}
              <div className="flex items-center text-amber-500 mt-1"> {/* Reduced mt */}
                <Star className="h-3.5 w-3.5 mr-1 fill-current" /> <span className="text-xs text-foreground font-poppins">{profile.rating.toFixed(1)}</span> {/* Reduced icon and text size */}
                 <span className="text-[0.65rem] text-muted-foreground ml-1">(Based on X reviews)</span> {/* Reduced text size */}
              </div>
              <CardDescription className="flex items-center justify-center text-muted-foreground text-xs pt-1"> {/* Reduced text size and pt */}
                <MapPin className="h-3 w-3 mr-0.5" /> {profile.address} {/* Reduced icon size */}
              </CardDescription>
               <div className="pt-2 flex flex-wrap justify-center gap-1.5"> {/* Reduced pt and gap */}
                    {Array.isArray(profile.specialty) ? profile.specialty.map(spec => (
                        <Badge key={spec} variant="secondary" className="bg-primary/10 text-primary text-[0.65rem] px-2 py-0.5 rounded-full shadow-sm font-poppins">{spec}</Badge> /* Reduced text size, px/py */
                    )) : (
                        <Badge variant="secondary" className="bg-primary/10 text-primary text-[0.65rem] px-2 py-0.5 rounded-full shadow-sm font-poppins">{profile.specialty}</Badge> /* Reduced text size, px/py */
                    )}
                </div>
            </CardHeader>
            
            <CardContent className="space-y-3 p-5"> {/* Reduced space-y and p */}
               {profile.yearsExperience && (
                <div className="flex items-center text-xs text-foreground"> {/* Reduced text size */}
                    <Award className="h-3.5 w-3.5 mr-1.5 text-primary/70"/> {/* Reduced icon size and mr */}
                    <span>{profile.yearsExperience} years of master craftsmanship</span>
                </div>
               )}
                {profile.certifications && profile.certifications.length > 0 && (
                    <div className="text-xs text-foreground"> {/* Reduced text size */}
                        <Sparkles className="h-3.5 w-3.5 mr-1.5 text-primary/70 inline"/> {/* Reduced icon size and mr */}
                        Certifications: {profile.certifications.join(', ')}
                    </div>
                )}
                {profile.responseTime && (
                    <div className="flex items-center text-xs text-foreground"> {/* Reduced text size */}
                        <MessageSquare className="h-3.5 w-3.5 mr-1.5 text-primary/70"/> {/* Reduced icon size and mr */}
                        <span>Responds: {profile.responseTime}</span>
                    </div>
                )}
                 {profile.ordersCompleted !== undefined && (
                    <div className="flex items-center text-xs text-foreground"> {/* Reduced text size */}
                        <Edit3 className="h-3.5 w-3.5 mr-1.5 text-primary/70"/> {/* Reduced icon size and mr */}
                        <span>{profile.ordersCompleted}+ Orders Completed</span>
                    </div>
                )}
               <Button size="default" className="w-full shadow-md rounded-full text-xs py-2.5 bg-primary hover:bg-primary/90 text-primary-foreground mt-1.5" onClick={handleRequestCustomOrder}> {/* Reduced size, py and mt */}
                  <Send className="mr-1.5 h-3.5 w-3.5"/> Request Custom Order {/* Reduced icon size */}
               </Button>
                <Button variant="outline" size="default" className="w-full border-primary text-primary hover:bg-primary/10 rounded-full text-xs py-2.5 shadow-sm" onClick={handleRequestIntroduction}> {/* Reduced size and py */}
                  <User className="mr-1.5 h-3.5 w-3.5"/> Request Introduction {/* Reduced icon size */}
               </Button>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="md:col-span-2 space-y-6"> {/* Reduced space-y */}
            <Card className="shadow-xl border-primary/10 rounded-xl bg-card">
                <CardHeader className="p-5"> {/* Reduced p */}
                    <CardTitle className="text-lg text-foreground">About {profile.name}</CardTitle> {/* Reduced text size */}
                </CardHeader>
                <CardContent className="p-5 pt-0"> {/* Reduced p */}
                    {profile.bio && <p className="text-sm text-foreground/85 leading-normal whitespace-pre-line">{profile.bio}</p>} {/* Reduced text size, leading */}
                </CardContent>
            </Card>

          <Card className="shadow-xl border-primary/10 rounded-xl bg-card">
            <CardHeader className="p-5"> {/* Reduced p */}
              <CardTitle className="text-lg text-foreground">Portfolio Showcase</CardTitle> {/* Reduced text size */}
               <CardDescription className="text-xs text-muted-foreground mt-0.5">A glimpse into the artisan&apos;s craft.</CardDescription> {/* Reduced text size and mt */}
            </CardHeader>
            <CardContent className="p-5 pt-0"> {/* Reduced p */}
                    ...
                    <div key={index} className="rounded-lg overflow-hidden shadow-md aspect-square group relative">
                        ...
                        />
                         <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                            <Eye className="h-7 w-7 text-white/80"/> {/* Reduced icon size */}
                         </div>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-muted-foreground text-center py-6 text-sm">This artisan&apos;s portfolio is currently being curated. Check back soon!</p> {/* Reduced py and text size */}
              )}
            </CardContent>
          </Card>
           ...
           <Card className="shadow-xl border-primary/10 rounded-xl bg-card">
            <CardHeader className="p-5"> {/* Reduced p */}
              <CardTitle className="text-lg text-foreground">Connect with {profile.name}</CardTitle> {/* Reduced text size */}
               <CardDescription className="text-xs text-muted-foreground mt-0.5">Initiate a conversation or request a custom piece through our secure admin-mediated process.</CardDescription> {/* Reduced text size and mt */}
            </CardHeader>
            <CardContent className="p-5 pt-0"> {/* Reduced p */}
              <form className="space-y-4" onSubmit={handleRequestIntroduction}> {/* Reduced space-y */}
                 ...
                 <Button type="submit" size="default" className="shadow-md rounded-full text-xs py-2.5 bg-primary hover:bg-primary/90 text-primary-foreground"> {/* Reduced size and py */}
                    <Send className="mr-1.5 h-3.5 w-3.5"/> Send Inquiry to Admin
                 </Button>
                 <p className="text-[0.65rem] text-muted-foreground pt-1.5">Your request will be reviewed by an administrator.</p> {/* Reduced text size and pt */}
              </form>
            </CardContent>
          </Card>
        </div>
