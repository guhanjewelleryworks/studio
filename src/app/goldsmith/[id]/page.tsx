...
import { MapPin, Star, MessageSquare, Send, Info } from "lucide-react";
...
export default function GoldsmithProfilePage({ params }: { params: Promise<PageParams> }) {
  ...
  if (isLoading) {
     return (
      <div className="container py-8 px-4 md:px-6"> {/* Reduced py */}
        <div className="grid md:grid-cols-3 gap-6"> {/* Reduced gap */}
          <div className="md:col-span-1 space-y-4"> {/* Reduced space-y */}
   ...
   if (error) {
     return <div className="container py-8 px-4 md:px-6 text-center text-destructive">{error}</div>; {/* Reduced py */}
   }

  if (!profile) {
    return <div className="container py-8 px-4 md:px-6 text-center text-muted-foreground">Profile not found.</div>; {/* Reduced py */}
  }
  ...
  return (
    <div className="container py-8 px-4 md:px-6"> {/* Reduced py */}
        <Alert variant="default" className="mb-6 border-accent text-accent bg-accent/5"> {/* Reduced mb */}
        ...
        <div className="md:col-span-1 space-y-4"> {/* Reduced space-y */}
          <Card className="shadow-lg border-primary/20">
            <CardHeader className="items-center text-center">
              <Avatar className="w-24 h-24 mb-3 border-2 border-primary"> {/* Reduced mb */}
              ...
              <CardDescription className="flex items-center justify-center text-muted-foreground">
               ...
               <div className="pt-2 flex flex-wrap justify-center gap-1"> {/* Reduced gap */}
            ...
            <Separator />
            <CardContent className="space-y-3 pt-4"> {/* Reduced space-y */}
        ...
        <div className="md:col-span-2 space-y-6"> {/* Reduced space-y */}
                  ...
                  {profile.portfolioImages.map((imgUrl, index) => (
                    <Image
                      ...
                      className="rounded-lg object-cover w-full aspect-[3/2] hover:opacity-90 transition-opacity cursor-pointer shadow-sm"
                       data-ai-hint="goldsmith portfolio jewelry"
                    />
                  ))}
                </div>
              ) : (
          ...
          {/* Contact Form Section - Now for Introduction Request */}
           <Card className="shadow-lg border-primary/20">
            ...
            <CardContent>
              <form className="space-y-3" onSubmit={handleRequestIntroduction}> {/* Reduced space-y */}
                 ...
                 <div className="space-y-1"> {/* Reduced space-y */}
                    ...
                    <Textarea id="contact-message" placeholder="Briefly explain why you'd like to connect..." required rows={4}/> {/* Reduced rows */}
                 </div>
                 <Button type="submit" className="shadow-sm">
                    <Send className="mr-2 h-4 w-4"/> Send Introduction Request
                 </Button>
                 <p className="text-xs text-muted-foreground pt-1">Your request will be reviewed by an administrator before contact is established.</p> {/* Reduced pt */}
              </form>
            </CardContent>
          </Card>
        </div>
