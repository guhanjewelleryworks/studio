...
import { Menu, Gem, LogIn, UserPlus } from 'lucide-react';
      ...
      <div className="container flex h-20 max-w-screen-xl items-center px-4 md:px-6">
        ...
        <Sheet>
          <SheetTrigger asChild>
           ...
           <SheetContent side="left" className="pr-0 bg-background w-[280px] sm:w-[320px] border-r border-border/20 shadow-xl">
             <div className="flex flex-col h-full">
              ...
              <nav className="flex-grow flex flex-col space-y-3 px-6 pt-6">
               ...
               <div className="px-6 pb-8 mt-auto flex flex-col gap-3 border-t border-border/20 pt-6">
                  <Link href="/login" className={cn(buttonVariants({ variant: "outline", size: "lg" }), "w-full rounded-full")}>
                    <LogIn className="mr-2 h-4 w-4" /> Login
                  </Link>
                  <Link href="/signup" className={cn(buttonVariants({ size: "lg" }), "w-full rounded-full bg-primary hover:bg-primary/90 text-primary-foreground")}>
        ...
        <div className="hidden md:flex flex-1 items-center justify-end space-x-3">
             <Link
                ...
                className={cn(
                   ...
                   'hover:bg-accent/10 text-foreground/80 hover:text-primary rounded-full px-6 py-2'
                )}
              >
                <span>Login</span>
             </Link>
